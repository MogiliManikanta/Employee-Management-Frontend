    ...
    const fetchLeave = async () => {
      try {
        const response = await axios.get(
          `https://employee-management-backend-uhoe.onrender.com/api/leave/detail/${id}`,
          {
            headers: {
  ...
  const changeStatus = async (id, status) => {
    try {
      const response = await axios.put(
        `https://employee-management-backend-uhoe.onrender.com/api/leave/${id}`,
        { status },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        <div className="max-w-4xl mx-auto mt-12 bg-white p-10 rounded-lg shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex justify-center items-center">
              <img
                src={`https://employee-management-backend-uhoe.onrender.com/${leave.employeeId.userId.profileImage}`}
                alt="Employee"
